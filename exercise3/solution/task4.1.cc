...
namespace scprog
{
  ...
  DualNumber add (DualNumber const& a, DualNumber const& b)
  {
    return DualNumber{.re=a.re+b.re, .ep=a.ep+b.ep};
  }

  DualNumber sub (DualNumber const& a, DualNumber const& b)
  {
    return DualNumber{.re=a.re-b.re, .ep=a.ep-b.ep};
  }

  DualNumber mul (DualNumber const& a, DualNumber const& b)
  {
    return DualNumber{.re=a.re*b.re, .ep=(a.re*b.ep + a.ep*b.re)};
  }

  DualNumber div (DualNumber const& a, DualNumber const& b)
  {
  ...
  DualNumber pow (DualNumber const& x, int n)
  {
    ...
    switch (n) {
      ...
      default: return mul(pow(x,n-1), x);
    }
  }
  ...
  bool test1 ()
  {
    ...
    bool pass1 = almost_equal( mul(_1,_eps), _eps );
    bool pass2 = almost_equal( mul(_eps,_1), _eps );
    bool pass3 = almost_equal( mul(_eps,_eps), _0 );
    bool pass4 = almost_equal( mul(_1,_1), _1 );
  ...
  bool test2 ()
  {
    ...
    auto f = add(sub(mul(_5, pow(_x,4)), mul(_2, pow(_x,2))), _7);
    auto df= sub(mul(_20, pow(_a,3)), mul(_4, _a));
